...
export const ProfilePage: React.FC = () => {
    const { currentUser, updateUser, users, assignCoOwner, removeCoOwner, upgradeToPlayerPlus } = useAuth();
    
    const [isEditing, setIsEditing] = useState(false);
    
    // State for general profile edits
    ...
    useEffect(() => {
        if (currentUser) {
            ...
            // Clear success message on user change
            setSuccess('');
        }
    }, [currentUser]);
    ...
    const handleUpgrade = async () => {
        setError('');
        setSuccess('');
        if (window.confirm('Are you sure you want to upgrade to a Player Plus account?')) {
            try {
                                ...
                                <div className="mt-6 flex items-center justify-center md:justify-start gap-4">
                                    <button onClick={() => setIsEditing(true)} className="bg-yellow-600 hover:bg-yellow-700 text-green-900 font-bold py-2 px-6 rounded-lg transition-transform hover:scale-105">
                                     ...
                                     {currentUser.role === 'player' && (
                                        <button onClick={handleUpgrade} className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-6 rounded-lg transition-transform hover:scale-105">
