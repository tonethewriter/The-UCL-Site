    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-green-950 px-4">
            <div className="max-w-md w-full bg-green-900/50 p-8 rounded-xl shadow-2xl border border-yellow-700/50">
                <h2 className="text-3xl font-bold text-center text-yellow-300 mb-6">Log In</h2>
                {error && <p className="bg-red-500/20 text-red-400 p-3 rounded-md mb-4 text-center">{error}</p>}
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        <label className="text-sm font-bold text-yellow-500 block mb-2" htmlFor="identifier">Gamertag or Email</label>
                        <input
                            type="text"
                            id="identifier"
                            value={identifier}
                            onChange={(e) => setIdentifier(e.target.value)}
                            className="w-full bg-green-800/60 text-yellow-200 border border-green-700 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-yellow-500 transition placeholder-yellow-600/50"
                            required
                        />
                    </div>
                    <div>
                ...
                <p className="text-center text-yellow-500 mt-6">
                    Don't have an account? <Link to="/signup" className="font-semibold text-yellow-400 hover:underline">Sign Up</Link>
                </p>
            </div>
        </div>
    );
