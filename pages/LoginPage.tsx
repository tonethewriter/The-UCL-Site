        ...
        <div className="min-h-screen flex items-center justify-center bg-brand-bg px-4">
            <div className="max-w-md w-full bg-brand-surface p-8 rounded-xl shadow-2xl border border-brand-border/50">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        <label className="text-sm font-bold text-brand-text-muted block mb-2" htmlFor="identifier">Gamertag or Email</label>
                        <input
                            type="text"
                            id="identifier"
                            value={identifier}
                            onChange={(e) => setIdentifier(e.target.value)}
                            className="w-full bg-black/30 text-white border border-brand-border rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-brand-accent transition placeholder-brand-text-muted/50"
                            required
                        />
                    </div>
                    <div>
                ...
                <p className="text-center text-brand-text-muted mt-6">
                    Don't have an account? <Link to="/signup" className="font-semibold text-brand-accent hover:underline">Sign Up</Link>
                </p>
            </div>
        </div>
    );
